...
// allow music background
function cancelAllowMusicBack() {
    document.querySelector(".allow-music").style.display = "none";
    music.pause()
    puse.classList.add("puse-act");
};

function allowMusicBackground() {
    if (music.play()) {
        document.querySelector(".allow-music").style.display = "none"
    }
};
...
function mainContent() {
    ...
    backMusic.innerHTML =
        "<audio id='music' loop src='audio/Art of Silence.mp3'></audio>";
...
years = new Date();
year.textContent = years.getFullYear();